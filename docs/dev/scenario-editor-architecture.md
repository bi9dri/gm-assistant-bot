...
| D17 | 投票 | **見送り** (#249)。スコープ外に移した |
| D18 | 配役 | **見送り** (#249)。スコープ外に移した |
...
各フェーズを #213 のサブ issue とする。順序は 1 → 2 → 3 → 4。0 は独立。
...
| 項目 | 理由 |
|------|------|
| 投票の集計・セッション用ロールの配役 (D17 / D18) | 現時点では着手しない (保留)。唯一 `backend/` を触る項目で、他のフェーズから独立している。必要になった時点で改めて issue を立てる |
